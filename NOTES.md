...
.dv ファイル (mmap)
  ...
  │    - 音声は WAV 一時ファイル経由 (PCM BE→LE 変換)、ALAC エンコード対応
  │    - 映像+音声を mux して最終出力
  │
  └─ ESDVCLI           コマンドラインインターフェース
       - info / convert サブコマンド
       - ストリーミングパイプライン (1フレームずつデコード+書き込み、~3MB/frame)
       - ビットストリームエラー検出・該当フレーム番号の警告出力
```
...
### 3. ACループのEOB未消費 → coefIdx==64時のVLC未消費(致命的)
- **旧**: `while coefIdx < 64`で全係数充填後にループ終了 → 末尾のEOB(4bit)未消費
- **修正1**: `while true` + EOB breakの無限ループ(smdn.jp: `for (i=1;;)`と同等)
- **修正2**: `coefIdx += 1`で64に到達した際のpeekによるEOB消費を追加 → 不十分
- **修正3(最終)**: `coefIdx += 1`後のチェックを削除し、ループ先頭に自然に戻す
  - リファレンス`for (i=1;;)`では`i++`後に明示的な境界チェックをせず、ループ先頭で次のVLCを読み取り（ビットを消費し）、EOBなら正常終了、非EOBなら`i += run >= 64`でrun length errorとして終了する
  - 修正2のpeek方式では、次のVLCが非EOBの場合にそのビットを消費せず、後続ブロックのDC係数読み取りが数ビットずれてビットストリーム同期を喪失していた
...
### ビットストリームエラーからの回復
- ソースファイルに不正なrun length等のビットストリームエラーが含まれる場合、該当ブロック以降のビットリーダー位置がずれて連鎖的にノイズが発生する
- DV VLCにはリージョン内の再同期マーカーがないため、エラー発生後のリカバリーは困難
- smdn.jpリファレンス実装でも同一のエラーが同一フレームで発生することを確認済み（ソースデータ起因）
- 現状はエラー検出と該当フレーム番号の警告出力のみ
...
Sources/esdv/
  ...
  ESDVDecoder.swift   VLC デコード、逆量子化、IDCT (行-列分離法)、YUV422p 出力、ビットストリームエラー検出
  FFmpegPipe.swift    ffmpeg プロセス管理、映像パイプ、音声 WAV/ALAC、mux、12 出力フォーマット対応
```
