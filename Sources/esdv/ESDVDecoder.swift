...
final class ESDVDecoder {
    ...
    private func decodeDCTBlock(
        ...
        // AC 成分: DV VLC デコード
        // smdn.jp リファレンス準拠:
        //   - 無限ループで EOB のみ終了 (for (i=1;;))
        //   - i >= 64 はエラー (run length error)
        //   - level=0 でも常に block[i] を書き込んで i++ する
        //   - coefIdx == 64 の時にも次の VLC (EOB) を必ず消費する
        var coefIdx = 1
        while true {
            ...
            if coefIdx >= 64 {
                // リファレンス: "run length error" → return -1 → caller continues
                // ビットストリームの同期は失われるが、処理を続行
                break
            }
