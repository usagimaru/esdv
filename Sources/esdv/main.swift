...
final class ESDVCLI {
	...
	private func runConvert(args: [String]) {
		...
		var audioCodec: AudioCodec = .wave
		...
		while i < args.count {
			...
			case "-a", "--audio":
				i += 1
				guard i < args.count else { die("--audio の後に値が必要です") }
				guard let a = AudioCodec(rawValue: args[i]) else {
					die("不明な音声コーデック: \(args[i])\n利用可能: \(AudioCodec.allCases.map(\.rawValue).joined(separator: ", "))")
				}
				audioCodec = a
			case "-r", "--fps":
				i += 1
	...
	private func convertFile(
		...
		print("  出力: \(output.lastPathComponent) [\(format.description)] 音声: \(audioCodec.description)")
		...
		var totalBitstreamErrors = 0
		var errorFrameIndices: [Int] = []
		...
		for chunkStart in stride(from: 0, to: frames.count, by: chunkSize) {
			...
			var decoders = (0..<chunkCount).map { _ in ESDVDecoder(fileHeader: header) }
			var decoded = [DecodedVideoFrame?](repeating: nil, count: chunkCount)
			decoders.withUnsafeMutableBufferPointer { decBuf in
				decoded.withUnsafeMutableBufferPointer { frameBuf in
					let decBase = decBuf.baseAddress!
					let frameBase = frameBuf.baseAddress!
					DispatchQueue.concurrentPerform(iterations: chunkCount) { i in
						frameBase[i] = try? decBase[i].decodeFrame(frames[chunkStart + i])
					}
				}
			}
			...
			for i in 0..<chunkCount {
				...
				if decoders[i].bitstreamErrorCount > 0 {
					totalBitstreamErrors += decoders[i].bitstreamErrorCount
					errorFrameIndices.append(globalIdx + 1)
				}

				guard let video = decoded[i] else {
					let d = ESDVDecoder(fileHeader: header)
					_ = try d.decodeFrame(frames[globalIdx])
					fatalError("unreachable")
				}
				try pipe.writeVideoFrame(video)

				let frame = frames[globalIdx]
		...
		if totalBitstreamErrors > 0 {
			fputs("警告: ソースファイルのビットストリームに \(totalBitstreamErrors) 件のエラーを検出しました\n", stderr)
			fputs("  該当フレーム: \(errorFrameIndices.map(String.init).joined(separator: ", "))\n", stderr)
			fputs("  該当フレームの映像が正しくデコードされていない可能性があります\n", stderr)
		}
	...
	private func writeRawYUVStreaming(
		...
		for chunkStart in stride(from: 0, to: frames.count, by: chunkSize) {
			...
			var decoders = (0..<chunkCount).map { _ in ESDVDecoder(fileHeader: fileHeader) }
			var decoded = [DecodedVideoFrame?](repeating: nil, count: chunkCount)
			decoders.withUnsafeMutableBufferPointer { decBuf in
				decoded.withUnsafeMutableBufferPointer { frameBuf in
					let decBase = decBuf.baseAddress!
	...
	private func printUsage() {
		...
		convert オプション:
		  ...
		  -a, --audio <codec>     音声コーデック (デフォルト: wave)
								  wave         … PCM 無圧縮 (WAV)
